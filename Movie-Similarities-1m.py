...
def makePairs(xxx_todo_changeme):
    (user, ratings) = xxx_todo_changeme
    (movie1, rating1) = ratings[0]
    (movie2, rating2) = ratings[1]
    return ((movie1, movie2), (rating1, rating2))

def filterDuplicates(xxx_todo_changeme2 ):
    (userID, ratings) = xxx_todo_changeme2
    (movie1, rating1) = ratings[0]
    (movie2, rating2) = ratings[1]
...
conf = SparkConf() #emtpy here, pass as arguments
sc = SparkContext(conf = conf)

print("\nLoading movie names...")
nameDict = loadMovieNames()

data = sc.textFile("s3n://sundog-spark/ml-1m/ratings.dat") #from cloud
...
if (len(sys.argv) > 1):
    ...
    results = filteredResults.map(lambda pair_sim1: (pair_sim1[1], pair_sim1[0])).sortByKey(ascending = False).take(10)
